...
## Очень.длинные.выражения.да.чтоб.в.одну.строчку()
Я называю их поездами в виду того, что в мире Java они имеют символическое
значение. В Scala практически все является выражением, и даже если что-то
возвращает `Unit` вы всегда можете получить на выходе ваш `()`. После
...
Который был разбит на именованные составляющие:
...
погрузился в язык, перегрузка операторов использовалась повсюду. Всеми везде
...
составлять DSL, такие, как в Parboiled или в akka-http.

Не перегружайте операторы без надобности, и если считаете что эта надобность
у вас есть все-равно не перегружайте.
