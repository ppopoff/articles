...
Многие из них до этого всегда работали лишь с императивными языка,
такими как C, C++ или Java, поэтому идиомы Scala оказываются для них непонятными
и, более того, неочевидными.
...
обучения сотрудников. В данный момент я вместе с другими
...
> красивым заглавием «Scala for juniors and junior seniors».
...
> но и вообще на всех, кто впервые знакомится с языком Scala,
> какой бы большой опыт императивного программирования у них ни был
> до этого знакомства.

Эта статья, по большому счёту, представляет собой солянку из практических
советов, поэтому она лишена какой-либо сложной многоуровневой академической
структуры изложения материала.
Вместо этого статья разбита на две части:
в первой части мы поговорим об идиомах функционального программирования
в Scala, а во-второй — обсудим объектно-ориентированные идиомы.
И начнем мы с наиболее недооцененной
возможности Scala — псевдонимов типов (type aliases).


### О псевдонимах типов
Многим начинающим разработчикам, не имевшим доселе опыта с `typedef`, эта
возможность языка кажется бесполезной. Однако, это совсем не так:
...
но добавление псевдонима для указателей на функцию-обработчик сигнала
решает эту проблему:
...
вдобавок они позволяет не сойти с ума от полных имён шаблонных типов.
...
И этим дело не ограничивается, вы Ocaml и SML вы также можете создавать union
типы (типы-объединения):
...
`type` ограничивается синонимизацией типов.
Для чего вообще стоит давать уже
известным типам другие имена? Во-первых это добавляет дополнительную
семантическую нагрузку, и, например, смысл типа DateString
становится более понятным чем
просто String, а `Map[Username, Key]` выглядит лучше, чем
`Map[String, String]`.
Во-вторых, синонимизация позволяет сократить большие и сложные
сигнатуры типов: `Map[Username, Key]` конечно неплохо, однако
`Keystore` намного короче, и понятней.

Конечно же у типов-синонимов есть и свои недостатки:
Вы видите тип Person, и не можете понять, объект это,
класс или псевдоним.

Злоупотреблять данным средством определенно не стоит,
однако есть ряд ситуацией
...
себе сервер возвращает `Option[List[Message]]`. Есть все хорошо, получаем
список сообщений внутри `Some`, если сообщений нет, получаем пустой список
внутри `Some`, если на сервере произошла ошибка, получаем `None`. Разумно?
Вполне, вот только зачем использовать для указания состояния ошибки `Option`,
когда у нас есть `Try` и `Either`?
Список может быть пустым так же, как и `Option`, поэтому можно спокойно
передавать пустой список, если что-то пойдет не так. Если вы хотите указать
на какие либо трудности, используйте `Try` или `Either`.
...
в новый тип, например,
используя `newtype` в [Haskell](https://wiki.haskell.org/Newtype),
в результате получая новый тип, о реализации которого пользователю ничего
не известно.
В чисто функциональных языках без кортежей никуда: они позволяют замечательно
представлять словари (dictionaries), без них было
[конволюция](https://en.wikipedia.org/wiki/Convolution_(computer_science)) была
бы менее наглядной.
...
По результатам [этого](https://www.scala-lang.org/old/node/5496) опроса
сделать это могут всего 7% Scala-разработчиков. Нижнее подчеркивание
используется в языке многократно и в различных контекстах.
...
не получится: импорты, случай по умолчанию при сопоставлении с образцом,
частично-применимые функции. Однако, внутри лямбда-выражений читаемости они
не добавляют Сложности в чтении лямбд, возникают при количестве
аргументов-подчеркиваний, превышающем 2.

При сопоставлении с образцом, они также способны попортить жизнь.
Вам понятно, что из себя представляет Fork и Leaf?
...
Да, эти значения не используются. Но это не тот случай когда их надо затирать
нижним подчеркиванием. Поверье, скорость программирования не упирается в
скорость набора текста, можно написать несколько лишних символов, в угоду
читаемости.
...
статья вам понравилась. Спасибо за то, что набрались терпения и дочитали это
