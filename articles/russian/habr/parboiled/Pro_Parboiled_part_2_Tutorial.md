...
В качестве примера напишем парсер, который ничего не делает, но тем не менее это не мешает ему существовать и радоваться жизни:
...
количество повторений заранее не известно, вам поможет следующая пара правил.

### zeroOrMore

Как вы уже наверное догадались из названия,(?) сопоставляет последновательность из нуля и более вхождений указанного
правила.(не правильно построено предложение - что сопоставляет ? название ?) Внимательный читатель уже заметил это правило в примерах и оно ему, скорее всего, показалось хорошо знакомым: в
...
в подобном операторе, в них этот факт записывается непосредственным образом, так же, как и в некоторых вариантах BNF. Для
...
Как видите, правило максимально упрощено, и я прекрасно отдаю отчет тому, что у нас не может быть 99 дней и 99 месяцев.
...
Так что, если у вам нужно разобрать выражение на языке C, начинать перечисление нужно с самых длинных операций, чтобы
они сопоставились первыми как предписывает стандарт. Упрощённо правило может выглядеть, например, так:
...
(корневого) правила. Если вы пишете модульный-тест для парсера, то возможно, имееть смысл вызывать этот метод и для
других правил. Скобочки после метода при этом обязательны.

Давайте заставим работать наш бесполезный парсер, умеющий сопоставлять только одну строковую константу. Итак, если(?)(нужно переписать - у тебя есть слово "если"  должно быть и "то" , перечитай до конца пункта и поймешь , смысл этого предложения потерялся , или кто-то что отсюда стер или ниже )
...
В качестве примера будет использоваться формат «BKV», который был придуман специально для этого туториала. Он(?кто он? туториал ? ты ? или формат ? )
...
программирования. Идентификатор может (?) (и\или???)должен начинаться либо с буквы английского алфавита (регистр не имеет значения),
...
с цифры, в виду наличия целочисленных литералов, различные символы могут легаться валидными операторами. Правило
описывающее ключ, будет выглядеть так:

    def Key = rule { Identifier }

Для описания значения воспользуемся уже имеющимся правилом (для этого нам всего-то нужно подмешать в написанный нами ранее
трейт):
...
Наконец, всё необходимое для объявления блока целиком у нас есть, поэтому объявим блока:(?) (объявим блока ? это который Александ Сергеевич ?)(не знаю тут будет блоки или блоки , по-этому и не исправила)
