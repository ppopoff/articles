...
В качестве примера напишем парсер, который ничего не делает, что не мешает ему существовать и радоваться жизни:
    ...
    class MyParser(val input: ParserInput) extends Parser {
      // Здесь описывается ваша грамматика
    }
...
Порядок перечисления вариантов в правиле `Signum` имеет значение: выбирается самый первый из подходящих вариантов, что
исключает возможность появления неоднозначности у грамматики. И да, так работают все без исключения PEG-парсеры.
Так что, если вам нужно разобрать выражение на языке C, начинать перечисление стоит с самых длинных операций,для того чтобы
они сопоставились первыми как и предписывает стандарт. Упрощенно правило может выглядеть, например, так:
