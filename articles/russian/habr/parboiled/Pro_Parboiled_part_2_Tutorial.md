...
Scala 2.11, однако существует артефакт и для 2.10.
...
Подробнее о правилах (или «продукциях», если заумные академические термины вам нравятся больше) будет рассказано
...
В Parboiled существуют два особенных терминала, которые, по идее, должны обрабатываться специальным образом, отличным
от простого текста (поэтому их ещё называют синтаксическими предикатами):
...
узнаете, так как парсер будет считать, что входные данные ещё не закончились и для него пока ещё не поздно всё исправить.
...
это и CSV, и перечисление аргументов функции через запятую, и многие другое. В Parboiled2 парсинг таких
...
> **Предупреждение:**  
> Как и проверка регулярными выражениями, этот парсер отсечёт много легитимных электронных адресов
...
Это исключает возможность появление неоднозначности у грамматики. И да, так работают все PEG-парсеры. Так, если
...
если их навертеть, ваши правила будут читаться немногим лучше, чем регулярки. С их помощью мы можем упростить наш парсер
знаковых чисел:
...
Теперь где-нибудь в другом месте создадим два экземплляра парсеров и подадим им на вход разные данные:
    ...
    val p1 = new UselessParser("Hopelessness")
    val p2 = new UselessParser("Happiness")
...
Для того, чтобы написать парсер экранированных строк, необходимо определиться со следующими элементами
синтаксиса:
...
парсер ожидает один из разрешенных символов или экранированную последовательность, а экранированная последовательность
