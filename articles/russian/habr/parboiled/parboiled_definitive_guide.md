...
(скобочки обязательны). Для этого где-нибудь в коде вызовите:
...
>Прогон прави выполняется намного проще чем в первом Parboiled, в котором существует целый зоопарк (см раздел Отчет
об Ошибках) Parser Runnerов, которые приходилось дополнительно создавать вручную.

Так же можно распознавать строки вне зависимости от их регистра:

    // передаваемая в правило строка должна быть в нижем регистре
    def StringWithCaseIgnored = rule { ignoreCase("string") }

Правила, они же "продукции" (Rule) - особая форма материи (подробнее о них будет рассказано позже). Сейчас нам главное
знать что Rule0 сопоставляет вход с правилом и говорит нам совпало или нет. Все приведенные ниже правила имеют
данный Rule0. Мы его не указывали потому что вывод типов пока справляется.
...
и непринужденно, что будет продемонстрированно на последующих примерах. Единственное дополнительное усилие, которое от
вас требуется - явно объявить тип в в том месте где используется рекурсивыный вызов.

Чтобы продемонстрировать разбор рекурсивных структур данных, зачастую, преподносят простой калькулятор в качестве
примера. По моему мнению - калькулятор не нагляден. В качестве примера у нас будет простенький формат конфигурационных
файлов, состоящий на основе именованных блоков и пар "ключ-значение".
...
нее не стоит забывать (Привет пользователям Parboiled1).

Теперь подробнее про лямбду. Вид ее будет зависеть от количества и типизации захваченных объектов. Для только что
захваченных выражений лямбда будет принимать одно значение типа String. Хранить на стеке строку мне не очень хочется,
поэтому преобразуем ее в целое число. 

    def Number: Rule1[Int] = rule { capture(Digit.+) ~> (numStr => numStr.toInt) }
В этой ситуации поощряется использование фирменного плейсходера:
...
существует весьма неприятный баг

С одним аргументом мы разобрались. А что делать если их несколько? Как поведет себя лямбда? А поведет она себя просто
и вполне предсказуемо: представьте что Стек значений растет справа налево. Соответственно самое левое из захваченных
значений (первое на стеке) или, если вам будет проще (Значение соответствующее типу самого левого типа в дженерике)
будет первым аргументом лямбды и так далее. Напрмиер для Rule2[Int, String], лямбда параметры лямбда выражения будут
...
This results in a Rule[Int :: HNil, String :: HNil], i.e. a rule that pops one Int value off the stack and replaces it
with a String. Note that, while the parameter types to the action function can be inferred if they can be matched
against an "output" of the underlying rule, this is not the case for parameters that don't directly correspond to an
underlying output. In these cases you need to add an explicit type annotation to the respective action function
parameter(s).

Сахарок для case classов
TODO

TODO:
  - Вложенные парсеры...
  рассказать про то что операторов как парбойлд1 типа ~~> и др теперь их нет

  - Показать пример вынимания линейной структуры и генерации AST ссылка на JSONParser??
...
на это никак не повлюяло. Всего-то нужно использовать self-typed-reference:

    trait Numbers { this: Parser =>
      // your code
    }

Первым же делом подлежит устранению стандартная библиотечка примитивов (если таковая имеется). После этого меняются
правила.
