...
сопоставятся. Парсер просто не пойдет проверять дальше. Поэтмоу, чтобы "работало" пример должен быть записан так:
...
Используя различные комбинации правил, мы можем сопоставлять сложные грамматики. Но обычно, от парсера требуется
извлекать что-то полезное из сопоставившихся данных. Для этого и существуют так называемые Parser Actions.
Действия на парсерах позволяют извлекать данные из сопоставившихся правил, преобразовывать их в зависимости от степени
вашей испорченности, или же использовать эти данные ради какого-нибудь побочного эффекта (side effect).`
