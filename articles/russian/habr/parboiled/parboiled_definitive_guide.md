...
against an "output" of the underlying rule, this is not the case for parameters that dont directly correspond to an
underlying output. In these cases you need to add an explicit type annotation to the respective action function
parameter(s).


Ну и самый сладкий сахар, что предлатает оператор действия, для case classов. (Аналогично оператору `~~>` в Parboiled1)
...
## run
Наиболее универсальное действие парсера. Во многих случаях run ведет себя так же как оператор `~>`. Однако отличия тоже присустсвуют
...
Currently parboiled only ever parses up to the very first parse error in the input. While this is all thats required
