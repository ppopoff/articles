...
идею можно взять и перенять. И, по заверениям Матиаса данное решение не является аккуратным, так как полагается на
изменяемое соатояние. Возможно в будущем нас будет ждать что-то похожее на полноценный каркас для тестирования.

Я вижу два подхода к тестированию парсера:
 - Тестировать парсер как единое целое: использовать различные краеугольные случаи.
 - Тестировать каждое правило по отдельности

Эти подходы можно совмещать. Лично я предпочитаю писать тесты не на каждое правило, а сразу работать с парсером. В
[примерах](https://github.com/ppopoff/parboiled-talk/tree/master/src/test/scala/com/doingfp/bkv/parser) использованы оба
подхода.

Во многих грамматиках есть весьма неудобные моменты. Например, если говорить о BSD-подобном формате сообщений
