...
(Кат: Читать дальше про ска́лу и парсеры → )
...
Данный паттерн очень часто встречается при синтаксическом разборе, поэтому мы обязательно, и в подробностях его
рассмотрим.
...
Мы воспользуемся последним (найти его определение можно выше). Так же мы объявим правила для необязательных пробелов.
    ...
    val WhiteSpaceChars = " \n\t"
    def OptWS = rule { zeroOrMore(anyOf(WhiteSpaceChars)) }

В некоторых местах пробелов может быть несколько, в некоторых может не быть, а кое где пробелы должны быть
обязательными. Нашему формату обязательные пробелы не требуются.

В нашем случае Ключ представляет собой валидный идентификатор для большинства языков
программирования. Наш Ключ может содержать цифры, буквы английского алфавита (строчные и заглавные), а так же символ
подчеркивания `_`. Перед тем как объявлять ключ, объявим предикат, описывающий набор допустимых для него символов.

    val KeyCharacter = CharPredicate.AlphaNum ++ "_"
...
Которое всегда падает, при этом с тем сообщением, с каким вы пожелаете:
    ...
    def Foo = rule { "foo" | fail("Хуй!") }

Получите следующее сообщени об ошибке:
`Invalid input 'x', expected Хуй (line 1, column 1):`
