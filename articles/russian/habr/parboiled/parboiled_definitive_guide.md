...
Работавшим с Parboiled1 этот раздел можно просто пролистать (объяснения могут показаться им слишком подробными).
Тем кто знакомится с Parboiled2 - следует прочитать.


А теперь, заставим наш парсер что-нибудь распознать. И начнем мы с терминалов.
Терминалы - символы не требующие дополнительных определений.
Нетерминалы - это более сложные правила, которые выводятся через Терминалы или другие Нетерминалы.
...
На выходе мы получим объект типа Try.
Так же можно распознать строки вне зависимости от их регистра
...
вход с правилом и говорит нам совпало или нет.
...
И Parboiled1 заставлял делать это часто. Настолько часто, что в pb2 появился целый класс CharPredicate.
Теперь так делать не надо, ибо у нас есть CharPredicate. Думаю, что из названий будет понятно каким категориям
символов они соответствуют.
...
Приходилось замыкать правило "с другой стороны" указвая blacklist или whitelist.
    ...
    // Alphanumeric + нижнее подчергивание
    def ValidIndentifier = rule {
      CharPredicate.AlphaNum ++ "_"
    }

Полезно будет рассказать еще о двух правилах: anyOf и noneOf. Работают они очень на except и union для CharPredicate,
однако на всем пространстве символов (ANY). И главное - они работают быстрее. Эти функции принимают на вход строку,
состоящую из перечислений символов. Например:

    // Сопоставит, если символ является цифрой или одной из операций
    def DigitOrOperation = rule { anyOf("1234567890+-=/*") }

    // сопоставит все кроме пробела
    def EverythingExceptTheWhiteSpace = rule { noneOf(" ") }

Конечно же никто не будет матчить единичные символы. Поэтому передем к более сложным правилам:
...
Некоторые грамматики требуют жесткого диапазона символов, например
[от двух до пяти](http://www.chukfamily.ru/Kornei/Prosa/Ot2do5/Ot2do5.htm). Теперь так можно сделать:

    def FromTwoToFive = rule { 2 to 5 times CharPredicate.Digit }

Для Parboiled1 существует правило nTimes, которое сопоставлет жесткое число символов, и не позволяет сопоставить
диапазон их количества.
...
### Optional
Optional, я бы перевел это как "может быть" - это как zeroOrOne. Либо есть одно вхождение, либо вхождений нет совсем.
У каждой строки текста есть вполне себе оформленный конец, а именно "Символ перехода на новую строку" или просто
Newline. В разных семейства операционных систем он кодируется по разному. Например в Unix-подобных операционных
системах нужен всего символ "\n", тогда как в Windows нужна последовательность из двух симвловов: "\r" и "\n".
...
Это правило коварно!
Порядок следования правил имеет значение. Так работают все PEG парсер-генераторы.
...
Как видите: где-то читать проще, где-то сложнее.
