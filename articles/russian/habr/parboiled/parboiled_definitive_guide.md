...
(скобочки обязательны). Для этого где-нибудь в коде вызовете:
...
Правила, они же "продукции" (Rule) - особая форма материи (подробнее о них будет рассказано позже). Нам главное
...
На данном этапе мы написали простой распознаватель, исходный код которого находится [тут](TODO). Сопоставлять значения на
практике приходится весьма редко. А вот захватывать их "каждый раз". Итак перейдем к "захватывающим историям"

## Стек значений (Value Stack)
Прежде чем извлекать какие-либо данные при помощи правил, следует рассказать про одно из средств парсера в Parboiled. И
средство это называется Value Stack или Стек значений. Представляет он из себя простой стек, который выступает
временным хранилищем для действий (Parser Actions) над извлекаемыми при помощи правил объектами. Так же действия
можно использовать для "вычислений на месте", как это сделано в примере с [калькулятором](http://bit.ly/1iJg7zP).
...
-----------------------------------------------------
  ...
  рассказать про то что операторов как парбойлд1 типа ~~> и др теперь ихнет


## Типы правил
В Parboiled2 существуют следующие типы правил:

 - Rule0 - Отвечает на вопрос "Сопоставилось ли?". Не изменяет стек.
 ...
 - RuleN - Помещает N объектов В стек значений. Использует семантику Shapeless. Для работы с Parboiled2 Shapeless знать
   не нужно (хотя и будет полезно). Тип правила, отправляющего в стек значений 4 элемента будет выглядеть так:

    import shapeless._
    def root: RuleN[Seq[String] :: String :: Seq[String] :: String :: HNil] = ??? // Не буду я его реализовывать.

В Parboiled1 для каждого количества аргументов (от 0 до 7) существовало по правилу. проблема в том ч
