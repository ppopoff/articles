  ...
  - [Parboiled for Scala] — библиотека, теперь уже более известная как Parboiled1, появилась на свет после того, как
	...
	со счетов ее пока что не стоит:
      ...
      - Parboiled1 всё ещё используется гораздо шире, чем Parboiled2, поэтому если вас внезапно перебросят на
	    какой-нибудь давно существующий Scala-проект, высок шанс столкнуться с ним.
...
Я писал эту статью с упором на Parboiled2, но иногда я буду отвлекаться, чтобы рассказать об важных отличиях между
первой и второй версиями.


## Основные фичи Parboiled2

  - Основан на [PEG](https://en.wikipedia.org/wiki/Parsing_expression_grammar).
...
[hier]:  https://en.wikipedia.org/wiki/Chomsky_hierarchy#The_hierarchy
  ...
  - В отличии от генераторов парсеров, таких как ANTLR, вы освобождены от мороки с раздельной генерацией кода и
    ...
    и проверку типов из коробки, так же как и отсутствие дополнительных операций над файлами грамматик, вто время
	как парсер, сгенерированный ANTLR будет иметь две фазы синтаксического разбора. Правда, не несмотря на это,
	ANTLR всё равно может оказаться мощнее, документированее и стабильнее, и поэтому предпочтительнее во многих
  ...
  - Скаловские парсер-комбинаторы работают медленно. Очень медленно. Просто издевательски медленно. Маттиас проводил
    ...
    Combinators. С неутешительными результатами можно для последних можно ознакомиться дальше по тексту.
  - В отличие от [Language Workbenches][lwb], Parboiled — маленькая и простая в использовании библиотека. Вам не нужно
    ...
    текстовый редактор с подсветкой вашего DSL из коробки и вам придется самостоятельно написать плагин для Vim, Emacs
	или вашей IDE, но это не делает Parboiled менее достойной альтернативой для разработки небольших
...
[bench-elv]: http://myltsev.name/ScalaDays2014/#/
      ...
      Парсер               │ Время, мс │ Хвост для сравнения
    ───────────────────────┼───────────┼─────────────────────────────────
