...
Данный раздел будет в основном полезен тем кто уже работал с первой версией библиотеки.
  ...
  - Parboiled2 решает ряд детских болезней первой версии:
    - Невозможность использования правил, более вместительных, чем Rule7. Для этого была использована библиотека1
      shapeless с ее знаменитыми HListами: теперь одно правило может оперировать большим количеством значений на стеке.
    - Добавлены недостающие конструкции. Например, В Parboiled нельзя было указать динамическое количество повторений
      для правила nTimes. Для решение приходилось использовать правило более "мягкое" правило oneOrMore, что не давало
...
Подробнее о правилах (Rules) (или "продукциях", если вам так нравится больше) будет рассказано далее.
...
А вот Parboiled 1 заставляет делать это часто - практически каждый раз, когда вы пишете парсер. Поэтому я носил свою
...
Приходилось замыкать правило "с другой стороны" указвая "черный" или "белый" список, в зависимости от ситуации.
    ...
    // Данный снипет будет работать медленно. Определите CharPredicate единожды
...
Для Parboiled 1 существует правило nTimes, которое сопоставлет жестко заданное число символов.
...
### Первый прогон
Прогонять следует главное (корневое) правило (если вы конечно не пишете модульные тесты). Давайте осуществим наш первый
...
В качестве примера будет использоваться формат BKV, который был придуман специально для этого туториала.
Возможно подобный формат существует и даже имеет имя. Пожалуйста дайте мне знать, если это так :).
Итак, формат BKV состоит из блоков, и пар ключ-значение. Выглядит это примерно так:
...
Данный паттерн очень часто встречается в большинстве грамматик с которыми вам возможно придется столкнуться. Поэтому,
предлагаю начать именно с него.
...
Мы воспользуемся последним (найти его определение можно выше). Так же объявим правила для необязательных пробелов.
    ...
    val WhiteSpaceChars = " \n\t"
    def WhiteSpace = anyOf(WhiteSpace)
    def MayBeWS = rule { zeroOrMore(anyOf(WhiteSpaceChars)) }

Очень часто можно встретиться с аналогичной ситуцией - грамматика требует наличя хотя бы одного пробела, если больше -
не страшно. Подобная задача решается аналогичным образом, просто вместо `zeroOrMore` используется `oneOrMore`.
...
Имея описаный набор допустимых символов нам не составит труда описать Ключ:
    ...
    def Key = rule { oneOrMore(KeyCharacter) }
...
([AST](https://en.wikipedia.org/wiki/Abstract_syntax_tree)). Так же действия можно использовать для "вычислений на
месте", как это сделано в примере с [калькулятором](http://bit.ly/1iJg7zP).
...
И с этого момента наше правило уже не Rule0. Данные захваченны и оправленны в стек значений. Впрочем, тип можно было не
указывать, компилятор и так все поймет сам.

### "Оператор действия ~> ()"
... или оператор, которым вам придется пользоваться чаще всего. В качестве параметра принимает лямбду. Может снимать
захваченные объекты cо стека и позволяет вам с этими объектами работать. Потом, при желании, значения можно отправить
обратно в стек, или же создать ноду для вашего AST. Это уже зависит от вашего желания. Для того чтобы действие
осуществилось, предварительно должен быть выполнен захват при помощи функции `capture`. В завимисости от типа
возвращаемого значения способен принимать различные формы. Это делает использование данного оператора интуитивным, и
простым.

> В Parboiled1 захват выполнялся неявно, что мне кажется весьма неудобным

Теперь подробнее про лямбду. Вид ее будет зависеть от количества и типизации захваченных объектов. За раз, лямбда может
захватить [не более 22 аргументов](https://github.com/sirthias/parboiled2/issues/85). При наличии одного захваченного
правила, лямбда будет принимать одно значение типа String. Хранить на стеке строку мне не очень хочется, поэтому
...
### run
Наиболее универсальное действие парсера. Во многих случаях run ведет себя так же как оператор `~>`. Одно из отличий
заключается в том что, в случае с run компилятор не выводит типы, поэтому их придется обозначать явно. Данная функция
очень удобна для создания непроверяемых сайд эффектов, это мы и рассмотрим на следующем примере:

    def RuleWithSideEffect = rule {
      capture(oneOrMore(CharPredicate.Printable)) ~> run { s: String => sideEffect(s) }
    }
    
Типом результирующего правила будет Rule0. Сопоставленная строка попадет в некий метод, вызывающий побочный эффект.
Зачастую бывает необходимо сопоставить текст минуя стек значений. Пользователи Parboiled1 наверное заметили, что в
данном контексте `run` ведет себя так же как оператор `~%`.

>При использовании сайд-эффектов, пожалуйста не заигрывайте со стеком значений. Да, к нему можно прямой доступ.
И по ряду причин этого лучше не делать.
...
Узлы между собой различать тоже как-то нужно. Поскольку у нас всего два типа нод мы можем добавить флаг говорящий о том
что мы имеем дело с блоком. Выделим общее из двух узлов:
...
оператором), способный возвращать следующую ноду.

    // Подмешайте этот трейт в AstNode
    trait NodeAccessDsl { this: AstNode =>
      ...
      // У перегуженных методов всегда должны быть надлежаще
      // проименованные альтернативы
      def / (nodename: String) = get(nodename)
    }

Можно реализовать огромное количество методов повышающих ваше удобство: рекурсивный поиск, возможность изменять значения
в нодах (если вам нужно изменяемое состояние). Эта техника заметно упрощает жизнь в будущем.
  
# Отчеты об ошибках
Не думаю, что вам захочется иметь парсер, который будет выдавать невнятные ошибки при при некорректных входных данных.
Parboiled2 вполне неплохо может рассказывать об ошибках. Конечно же, если вы ему в этом поможете.

## Форматирование
Итак, если что-то "навернулось", в ваше распоряжение поступит объект типа ParseError, который можно привести в божеский
вид как-то так:

    val errorMessage = parser formatError error
    
Можно так же использовать явное форматирование:

    val errorMessage parser.formatError(error, new ErrorFormatter(showTraces = true))
    
Сам же класс ParseError объявляется так:

    case class ParseError(position: Position, charCount: Int, traces: Seq[RuleTrace]) extends RuntimeException

так же стоит отметить наличие различных съем доставки: по вашему желанию конечный результат может быть представлен не
только в виде объекта Try, а например в виде полиморфного типа, или же Either. Подробнее можно ознакомиться
[здесь](https://github.com/sirthias/parboiled2/blob/master/README.rst#alternative-deliveryschemes)


## Тонкая настройка
### Для тех кто предпочитает ручную трансмиссию
Есть опция позвояющая обойти встроенный механизм очета об шибках. Для этого можно использовать правило fail,
Которое всегда падает, при этом с тем сообщением, с каким вы пожелаете:

    def Foo = rule { "foo" | fail("Я упаль!") }

Получите следующее сообщени об ошибке:
`Invalid input 'x', expected Я упаль! (line 1, column 1):`

## Именованные правила
Использование их бывает весьма полезным и не только в целях отлова ошибок. Использование этого механизма описано ниже
в разделе Best Practices.

## Atomic
Parboiled2 это парсер построенный на PEG, это означает что он работает на уровне символов, а не строк (как многие
могут подумать). Поэтому ошибки вам будут показываться на символьном уровне. Согласитесь, сообщение вида
"У вас тут X, мы ожидали или Y или Z", потребует больше мысленных усилий, чем: "У вас тут XX, а мы ожидали увидеть XY
или XZ". Для того чтобы была возможность видеть строки в отчетах об ошибках целиком придуман маркер `atomiс`.
Всего то и нужно обернуть в него нужное правило:
...
## Quiet
Используется когда вариантов для выбора слишком много и не хочется показывать их все. Например в определенном месте
ваш парсер ожидает множество пробельных символов в совокупе с неким правилом. Для устранения избыточности в отчете,
возможно, вы захотите умолчать о пробелах. С использованием маркера `quiet` это возможно:

    def OptionalWhitespaces = rule { quiet(zeroOrMore(anyOf(" \t\n"))) }
 
Честно признаюсь, ситуаций поощряющих использования данной настройки не встречал. Так же как и atomic подробно
[описан в документации](https://github.com/sirthias/parboiled2/blob/master/README.rst#the-quiet-marker).
...
# Тестирование
Разработчики Parboiled используют для тестирования фреймворк specs2. И у них уже имеется небольшой класс-помощник
[TestParserSpec](https://goo.gl/v5tCWS). Он покажется неудобным тем, кто использует scalatest. Но, основную
идею можно взять и перенять. И, по заверениям Матиаса данное решение не является аккутартным, так как полагается на
изменяемое соатояние. Возможно в будущем нас будет ждать что-то похожее на полноценный каркас для тестирования.

Правила можно тестировать как по отдельности так и вместе. Лично я предпочитаю писать тесты не на каждое правило, а
сразу рассматривать краеугольные случаи. Во многих форматах есть весьма неудобные моменты. Что я под этим понимаю:
например, если говорить о BSD-подобном формате сообщений [Rfc3164](https://www.ietf.org/rfc/rfc3164.txt), то под дату
там резервируется два разряда. У случае если у число в дате имеет один разряд (например 5 Августа), то первый символ
заменяется пробелом. Пример из Rfc:

>If the day of the month is less than 10, then it MUST be represented as a space and then the number.  For example,
the 7th day of August would be represented as "Aug  7", with two spaces between the "g" and the "7".

По мимо "краеугольных камней" можно тестировать незакрытые блоки, наличие недопустимых символов, а так же корректность
операций со стеком значений. Предположим, вы хотите оттестировать следеющее правило:

    def Decimal = rule { ('+' | '-').? ~ Digit.+ ~ "." ~ Digit.+ }
    
При прогоне теста, мы всегда получим удачный результат, и наш тест упадет:

    // Я еще не видел десятичных дробей с двумя разделителями
    val p = new MyParser("12.3.456").Decimal.run()
    p.isFailure shouldBe true

Почему так? Мы забыли EOI. Чтобы добавить EOI нам нужно тестирующее правило. Для этой цели можно использовать хитрый
механизм [мета-правил](https://github.com/sirthias/parboiled2/blob/master/README.rst#advanced-techniques), можно создть
правило для каждого теста в отдельности.
...
У любой, даже самой замечательной библиотеки есть свои недостатки, Parboiled2 не является исключением.
 ...
 - Многие IDE еще не научились поддерживать макровыражения. Parboiled2 построен на макровыражениях. Поэтому верить
   подчеркиваниям вашей среды разработки не стоит. Сам, забыв об этом, потратил целый день на поиск ошибки в простом
   подпарсере.
 ...
 - Parboiled не доработан для поддержки лево-рекурсивных грамматик. Поэтому вам следует доработать вашу грамматику под
   Parboiled.
...
знать ограничения данного типа грамматик стоит. Итак, в виду того что Parboiled является PEG он не умеет:
 ...
 - Разбирать грамматики на отступах (Indentation-based grammars), например Python или YAML. Не получается это сделать
   ...
   этапе лексического анализа. Однако, для решения данной проблемы существует решение: пишете препроцессор и 
   устанавливаете виртуальные символы (маркеров) до (INDENT) и после (DEDENT) выхода в отступ. Для Parboiled1 сущестует
 ...
 - Использовать потоковый ввод (Streaming input). PEG используют поиск с возвратом, он же
   ...
   потоку. Данное решение, возможно, и окажется кому-то полезным. Матиас очень [заинтересован](http://bit.ly/1FNCQj5) в
   разработке этой фичи, так что возможно ее появление в следующих релизах
