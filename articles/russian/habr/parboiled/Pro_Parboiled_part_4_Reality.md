...
В этой статье будет рассказано о дополнительных возможностях, которые предоставляет Parboiled, о миграции,
техниках, и не достатках. Вобщем, о всем том, что так и не нашло место в предыдущих частях.
...
### atomic
Parboiled2 создает парсер, построенный на PEG. Это означает что он работает на уровне символов, а не строк (как многие
...
едва ли это число больше четырех.
...
со стека значений. Например, так можно применить ее к предыдущему правилу:
...
# Миграция
Миграция — процесс, чаще всего, несложный, однако времени отнимает немало. Поэтмому я постараюсь хотя бы немного
...
Что, кстати, работает весьма неплохо, при наличии нескольких автономных парсеров. Если же ваши парсеры сотоят
из множства модулей, переиспользуемых в разных местах (как это было в моем случае) — вам придется делать миграцию
...
библиотеку различные форматы дат, грамматику описывающую электронную почту, и заголовки HTTP. Parboiled просто
комбинатор парсеров: он таковым был, таким и останется. Однако согласитесь -- выбрасывать старый код очень приятно.
...
а в лучшем - станет руководстком к действию.
